...
struct DashboardWidget: View {
    ...
    init(icon: String, title: String, parentSize geometry: GeometryProxy, destination: (any View)?) {
        self.icon = icon
        self.title = title
    ...
    var body: some View {
        NavigationLink(destination: destination != nil ? AnyView(destination!) : nil) {
